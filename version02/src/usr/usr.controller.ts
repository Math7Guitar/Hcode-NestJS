...
export class UsrController {
  ...
  @Post()
  create(@Body() createUsrDto: CreateUsrDto): Promise<Usr> {
  ...
  @Get()
  find(@Query('id') id: string): Promise<Array<Usr>> | Promise<Usr> {
    if (id) {
  ...
  update(@Param('id') id: string, @Body() updateUsrDto: UpdateUsrDto): Promise<Usr> {
    return this.usrService.update(id, updateUsrDto);
  }

  @Role(Roles.ADMIN)
  @Delete(':id')
  remove(@Id() id: string): Promise<Usr> {
    console.log(id);
